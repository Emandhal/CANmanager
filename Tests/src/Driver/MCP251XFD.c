...
#ifdef __cplusplus
  pComp->GPIOsOutLevel = pConf->GPIOsOutLevel;                                                           // [C++] Transfer GPIOs output level at startup configuration, because the of the class
#endif
...
#endif
  Error = MCP251XFD_SetBitTimeConfiguration(pComp, ConfBitTime);                                         // Set Bit Time configuration to registers
  if (Error != ERR_NONE) return Error;                                                                   // If there is an error while calling MCP251XFD_SetBitTimeConfiguration() then return the error
...
#endif
      uint16_t FrameCRC = pComp->fnComputeCRC16(&Buffer[0], (ByteCount + 2 + 1)); // Compute CRC on the Buffer data (2 for Command + 1 for Length)
      uint16_t BufCRC = *pBuf << 8;                                          // Get CRC MSB on the next buffer byte
      ++pBuf;
      BufCRC |= *pBuf;                                                       // Get CRC LSB on the next buffer byte
      if (FrameCRC != BufCRC) return ERR__CRC_ERROR;                         // If CRC mismatch, then raise an error
    }
  }
...
//=============================================================================
// Set a pin on PORT direction of the MCP251XFD device
//=============================================================================
eERRORRESULT MCP251XFD_SetPinState_Gen(GPIO_Interface *pIntDev, const eGPIO_State pinState)
{
#ifdef CHECK_NULL_PARAM
  if (pIntDev == NULL) return ERR__NULL_POINTER;
#endif
  ...
  {
    default: break;
    case GPIO_STATE_OUTPUT: return MCP251XFD_SetGPIOPinsDirection(pDevice, 0x00, pIntDev->PinBitMask);
    case GPIO_STATE_INPUT : return MCP251XFD_SetGPIOPinsDirection(pDevice, pIntDev->PinBitMask, pIntDev->PinBitMask);
    case GPIO_STATE_RESET : return MCP251XFD_SetGPIOPinsOutputLevel(pDevice, 0x00, pIntDev->PinBitMask);
    case GPIO_STATE_SET   : return MCP251XFD_SetGPIOPinsOutputLevel(pDevice, pIntDev->PinBitMask, pIntDev->PinBitMask);
    case GPIO_STATE_TOGGLE: return MCP251XFD_SetGPIOPinsOutputLevel(pDevice, pDevice->GPIOsOutLevel ^ (uint8_t)pIntDev->PinBitMask, pIntDev->PinBitMask);
  }
...
//=============================================================================
eERRORRESULT MCP251XFD_CalculateBitTimeConfiguration(const uint32_t fsysclk, const struct CAN_CANFDbusConfig busConf, struct CAN_BitTimeConfig* const pConf)
{
#ifdef CHECK_NULL_PARAM
  ...
  if (busConf.DesiredNominalBitrate < MCP251XFD_NOMBITRATE_MIN ) return ERR__BAUDRATE_ERROR;
  if (busConf.DesiredNominalBitrate > MCP251XFD_NOMBITRATE_MAX ) return ERR__BAUDRATE_ERROR;
  if (busConf.DesiredDataBitrate != CAN_NO_CANFD)
    if (busConf.DesiredDataBitrate  < MCP251XFD_DATABITRATE_MIN) return ERR__BAUDRATE_ERROR;
  if (busConf.DesiredDataBitrate    > MCP251XFD_DATABITRATE_MAX) return ERR__BAUDRATE_ERROR;
  ...
  {
    uint32_t NTQbits = fsysclk / busConf.DesiredNominalBitrate/ BRP;                      // Calculate the NTQbits according to BRP and the desired Nominal Bitrate
    ...
    if (busConf.DesiredDataBitrate != CAN_NO_CANFD)
    {
      DTQbits = fsysclk / busConf.DesiredDataBitrate / BRP;                               // Calculate the DTQbits according to BRP and the desired Data Bitrate
    ...
    // NTQ & DTQ bits count
    ErrorNTQ = (fsysclk - (busConf.DesiredNominalBitrate * NTQbits * BRP));                                   // Calculate NTQ error
    if (busConf.DesiredDataBitrate != CAN_NO_CANFD)
    {
      if (ErrorNTQ == 0) ErrorNTQ = 1;                                                                        // Adjust NTQ error
      ErrorDTQ = (fsysclk - (busConf.DesiredDataBitrate * DTQbits * BRP)); if (ErrorDTQ == 0) ErrorDTQ = 1;   // Calculate DTQ error
    ...
    {
      ErrorNTQ = ((busConf.DesiredNominalBitrate * (NTQbits+1) * BRP) - fsysclk);                             // Calculate NTQ error with NTQbits+1
      if (busConf.DesiredDataBitrate != CAN_NO_CANFD)
      {
        if (ErrorNTQ == 0) ErrorNTQ = 1;                                                                      // Adjust NTQ error
        ErrorDTQ = (fsysclk - (busConf.DesiredDataBitrate * DTQbits * BRP)); if (ErrorDTQ == 0) ErrorDTQ = 1; // Calculate DTQ error
    ...
    if (busConf.DesiredDataBitrate != CAN_NO_CANFD)
    {
      ...
      {
        ErrorNTQ = (fsysclk - (busConf.DesiredNominalBitrate * NTQbits * BRP));  if (ErrorNTQ == 0) ErrorNTQ = 1;    // Calculate NTQ error
        ErrorDTQ = ((busConf.DesiredDataBitrate * (DTQbits+1) * BRP) - fsysclk); if (ErrorDTQ == 0) ErrorDTQ = 1;    // Calculate DTQ error with DTQbits+1
        ErrorTQ = (ErrorNTQ * ErrorDTQ);
        if (ErrorTQ <= MinErrorBR)                                                                                   // If better error then
      ...
      {
        ErrorNTQ = ((busConf.DesiredNominalBitrate * (NTQbits+1) * BRP) - fsysclk); if (ErrorNTQ == 0) ErrorNTQ = 1; // Calculate NTQ error with NTQbits+1
        ErrorDTQ = ((busConf.DesiredDataBitrate * (DTQbits+1) * BRP) - fsysclk);    if (ErrorDTQ == 0) ErrorDTQ = 1; // Calculate DTQ error with DTQbits+1
        ErrorTQ = (ErrorNTQ * ErrorDTQ);
        if (ErrorTQ <= MinErrorBR)                                                                                   // If better error then
  ...
  if (busConf.DesiredDataBitrate != CAN_NO_CANFD)
  {
    ...
    else pConf->TDCmode = CAN_TDC_MANUAL_MODE;                      // ** Set Manual; Don’t measure, use TDCV + TDCO from register
    const uint32_t SSP = BestBRP * DTSEG1;                          // In order to set the SSP, SSP = TDCO + TDCV (Equation 3-10 of MCP25XXFD Family Reference Manual). SSP is set to DBRP * (DPRSEG + DPHSEG1) = DBRP * DTSEG1
    uint32_t TDCO = SSP;
  ...
  if (pConf->Stats != NULL)
    Error = MCP251XFD_CalculateBitrateStatistics(fsysclk, pConf);   // If statistics are necessary, then calculate them
...
//=============================================================================
eERRORRESULT MCP251XFD_CalculateBitrateStatistics(const uint32_t fsysclk, const CAN_BitTimeConfig* const pConf)
{
#ifdef CHECK_NULL_PARAM
  if (pConf == NULL) return ERR__PARAMETER_ERROR;
  ...
  const uint32_t MinNPHSEG  = (NPHSEG1 <= (pConf->NTSEG2+1u) ? NPHSEG1 : (pConf->NTSEG2+1u));                                               // Get min(NPHSEG1, NPHSEG2)
  pConf->Stats->OscTolC1     = (((pConf->NSJW+1u) * 10000u) / (2u * 10u * NTQbits));                                                        // Condition 1 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-12 of MCP25XXFD Family Reference Manual)
  pConf->Stats->OscTolerance = pConf->Stats->OscTolC1;
  pConf->Stats->OscTolC2     = ((MinNPHSEG * 10000u) / (2u * (13u * NTQbits - (pConf->NTSEG2+1u))));                                        // Condition 2 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-13 of MCP25XXFD Family Reference Manual)
  ...
  {
    pConf->Stats->OscTolC3     = (((pConf->DSJW+1u) * 10000u) / (2u * 10u * DTQbits));                                                                                        // Condition 3 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-14 of MCP25XXFD Family Reference Manual)
    ...
    const uint32_t NBRP        = (pConf->NBRP+1u), DBRP = (pConf->DBRP+1u);
    pConf->Stats->OscTolC4     = ((MinNPHSEG * 10000u) / (2u * ((((6u * DTQbits - (pConf->DTSEG2+1u)) * DBRP) / NBRP) + (7u * NTQbits))));                                    // Condition 4 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-15 of MCP25XXFD Family Reference Manual)
    ...
    const int32_t NBRP_DBRP    = ((NBRP * 10000u) / DBRP), MaxBRP = ((NBRP_DBRP - 10000u) > 0u ? (NBRP_DBRP - 10000u) : 0u);                                                  // NBRP/DBRP and max(0,(NBRP/DBRP-1)). The use of 10000 is to set 2 digits on the C5 result
    pConf->Stats->OscTolC5     = ((((pConf->DSJW+1u) * 10000u) - MaxBRP) / (2u * (((2u * NTQbits - (pConf->NTSEG2+1u)) * NBRP) / DBRP + (pConf->DTSEG2+1u) + 4u * DTQbits))); // Condition 5 for the maximum tolerance of the oscillator with 2 digits after the decimal point (Equation 3-16 of MCP25XXFD Family Reference Manual) [WARNING: An error seems to be present in the original formula]
...
//=============================================================================
eERRORRESULT MCP251XFD_SetBitTimeConfiguration(MCP251XFD *pComp, const CAN_BitTimeConfig* const pConf)
{
#ifdef CHECK_NULL_PARAM
  if (pConf == NULL) return ERR__PARAMETER_ERROR;
#endif
  ...
  if (pConf->CAN20only == false)
  {
    ...
    TConfig.CiTDC = MCP251XFD_CAN_CiTDC_TDCO_SET(pConf->TDCO) | MCP251XFD_CAN_CiTDC_TDCV_SET(pConf->TDCV)              // Set Data Bit Time configuration
                  | MCP251XFD_CAN_CiTDC_TDCMOD_SET(pConf->TDCmode) | MCP251XFD_CAN_CiTDC_EDGFLTDIS;
    if (pConf->EdgeFilter) TConfig.CiTDC |= MCP251XFD_CAN_CiTDC_EDGFLTEN;                                              // Enable Edge Filter if asked
